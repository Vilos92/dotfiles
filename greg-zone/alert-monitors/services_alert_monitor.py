...
class ServicesAlertMonitor(BaseAlertMonitor):
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                logger.info(f"Checking logs from {self.last_check_time} to {current_time}")
    ...
    def process_suspicious_activity_alert(self, config: Dict[str, Any], logs: List[Dict]):
    ...
    def process_user_activity_alert(self, config: Dict[str, Any], logs: List[Dict]):
        ...
        for log in logs:
            ...
            if match:
                ...
                if should_alert:
                    logger.info(f"User activity detected: {username} using {config['service']}")
    ...
    def send_service_health_alert(self, config: Dict[str, Any], state: str):
        ...
        try:
            ...
            logger.info(f"Sent state alert: {config.get('state_key', config['name'])} {state}")
            
        except Exception as e:
            logger.error(f"Error sending service health alert: {e}")
    
    def send_suspicious_activity_alert(self, config: Dict[str, Any], ip_address: str, count: int, last_request_data: Dict):
    ...
    def send_ip_access_alert(self, config: Dict[str, Any], match_data: Dict, ip_address: str):
        ...
        try:
            ...
            discord_message = config['discord_message'].format(
                ...
                remote_addr=ip_address,
                location=location,
                formatted_time=format_nginx_timestamp(match_data.get('time_local', 'Recent')),
            ...
            self.send_webhook(payload)
            logger.info(f"Sent IP access alert: {config['service']} - {ip_address} - {service_name}")
    ...
    def send_user_activity_alert(self, config: Dict[str, Any], match_data: Dict):
        ...
        try:
            ...
            self.send_webhook(payload)
            logger.info(f"Sent user activity alert: {config['service']} - {match_data.get('username', 'Unknown')}")
