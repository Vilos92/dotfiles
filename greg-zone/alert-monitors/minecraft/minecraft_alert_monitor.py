...
class MinecraftAlertMonitor(BaseAlertMonitor):
    ...
    def process_server_health_check(self, config: Dict[str, Any]):
        ...
        logger.info(f"Server health changed for {state_key}: {previous_state} -> {current_state}")
    ...
    def send_server_health_alert(self, config: Dict[str, Any], state: str):
        ...
        try:
            ...
            logger.info(f"Sent server health alert: {config.get('state_key', config['name'])} {state}")
    ...
    def send_player_activity_alert(self, config: Dict[str, Any], match_data: Dict):
        ...
        try:
            ...
            self.send_webhook(payload)
            logger.info(f"Sent player activity alert: {config['service']} - {match_data.get('player_name', 'Unknown')} {match_data.get('event_type', 'unknown')}")
