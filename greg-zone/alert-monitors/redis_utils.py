...
class AlertMonitorRedis:
    ...
    def get_ip_data(self, ip_address: str) -> Optional[Dict[str, Any]]:
        ...
        if ip_json:
            ...
            if 'cooldowns' in data:
                data['cooldowns'] = {k: datetime.fromisoformat(v) for k, v in data['cooldowns'].items()}
            return data
        return None
    ...
    def set_ip_data(self, ip_address: str, ip_data: Dict[str, Any]):
        ...
        if 'last_seen' in serializable_data and isinstance(serializable_data['last_seen'], datetime):
            serializable_data['last_seen'] = serializable_data['last_seen'].isoformat()
        if 'cooldowns' in serializable_data:
            serializable_data['cooldowns'] = {k: v.isoformat() if isinstance(v, datetime) else v
    ...
    def set_player_mapping(self, player_key: str, player_data: Dict[str, Any]):
        ...
        if 'updated_timestamp' in serializable_data and isinstance(serializable_data['updated_timestamp'], datetime):
            serializable_data['updated_timestamp'] = serializable_data['updated_timestamp'].isoformat()
        if 'last_online_timestamp' in serializable_data and isinstance(serializable_data['last_online_timestamp'], datetime):
            serializable_data['last_online_timestamp'] = serializable_data['last_online_timestamp'].isoformat()
    ...
    def update_ip(self, ip_address: str, last_seen: datetime, country: str = 'Unknown', 
                  is_suspicious: bool = False, alert_count: int = 0):
    ...
    def set_ip_cooldown(self, ip_address: str, alert_type: str, cooldown_until: datetime):
